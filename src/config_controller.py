...
class ConfigController(tk.Tk):
    ...
    def show_wizard_step(self, step):
        ...
        if step == 1:
            ...
            self.current_panel.pack(fill="both", expand=True)
            tk.Button(self.container, text="Siguiente: Backups ➔", command=lambda: self.show_wizard_step(3)).pack(
                pady=20)
    ...
    def validate_and_next(self, from_step, to_step):
        ...
        # Si la validación ha sido exitosa (o no requería validación), avanzamos de pantalla
    ...
    def on_closing(self):
        ...
        # Si estamos en el asistente inicial y NO hay una clave guardada en el sistema...
        if self.is_setup_wizard and not keyring.get_password("Gandalf_Guard", "MASTER_KEY"):
            ...
            # Si ya hay clave o no es el asistente de instalación, permitimos cerrar normalmente
            self.destroy()
